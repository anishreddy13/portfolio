...
export function useDashboardData(
  ...
  const fetchData = useCallback(async (pageNum = 1) => {
    ...
    try {
      ...
      const { data: versionsData } = await client
        ...
        .limit(10);
      const { data: driftData } = await client
        .from("drift_reports")
        .select("*")
        .order("created_at", { ascending: false })
        .limit(20);
  ...
  useEffect(() => {
    ...
    const predChannel = client.channel("pred_live")
    ...
    const driftChannel = client.channel("drift_live")
    ...
    const versionChannel = client.channel("version_live")
    ...
    const activityChannel = client.channel("activity_live")
    ...
    const pipelineChannel = client.channel("pipeline_live")
    ...
    return () => {
      client.removeChannel(predChannel);
      client.removeChannel(driftChannel);
      client.removeChannel(versionChannel);
