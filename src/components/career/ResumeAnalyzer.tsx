...
function LoadingProgress({ stepIndex }: { stepIndex: number }) {
  ...
  return (
    <div className="rounded-sm px-6 py-16 flex flex-col items-center justify-center min-h-[520px]" style={{ background: "var(--surface-1)", border: "1px solid rgba(255,255,255,0.06)" }}>
      ...
      <div className="w-full max-w-sm h-1.5 rounded-full overflow-hidden" style={{ background: "var(--border)" }}>
        <motion.div className="h-full rounded-full" style={{ background: "#C8FF00" }} animate={{ width: `${progress}%` }} transition={{ duration: 0.45 }} />
      </div>
      <p className="font-body text-xs mt-4 text-center max-w-xs" style={{ color: "var(--text-tertiary)" }}>
        ...
        {gaps.length ? gaps.map((gap) => (
          <motion.div key={gap.skill} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-sm p-3" style={{ background: "var(--surface-2)", border: "1px solid rgba(255,255,255,0.04)" }}>
            <div className="flex justify-between gap-3 mb-2">
              <span className="font-mono text-[0.62rem] uppercase" style={{ color: "var(--text-primary)" }}>{gap.skill}</span>
            ...
            <p className="font-body text-xs leading-relaxed mt-2" style={{ color: "var(--text-tertiary)" }}>{gap.reason}</p>
          </motion.div>
        )) : (
          <div className="rounded-sm p-5 text-center" style={{ background: "rgba(200,255,0,0.05)", border: "1px solid rgba(200,255,0,0.16)" }}>
            ...
            <p className="font-body text-xs mt-2" style={{ color: "var(--text-secondary)" }}>Your skills align well with market demand.</p>
          </div>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {quickWins.length ? quickWins.map((win) => (
            <div key={`${win.skill}-${win.relates_to}`} className="rounded-sm p-3" style={{ background: "rgba(200,255,0,0.06)", border: "1px solid rgba(200,255,0,0.16)" }}>
              ...
              <p className="font-body text-xs leading-relaxed" style={{ color: "var(--text-secondary)" }}>{win.why}</p>
              <p className="font-mono text-[0.5rem] uppercase tracking-widest mt-2" style={{ color: "var(--text-tertiary)" }}>Relates to {win.relates_to || "your current stack"}</p>
            </div>
          )) : (
            <p className="font-mono text-[0.55rem] uppercase tracking-widest" style={{ color: "var(--text-tertiary)" }}>Quick wins appear after market skills are available.</p>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
      <div className="rounded-sm p-4 sm:p-5 space-y-3" style={{ background: "var(--surface-1)", border: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <input value={name} onChange={(event) => setName(event.target.value)} placeholder="Your name" className="w-full rounded-sm px-3 py-3 bg-transparent font-mono text-sm focus:outline-none" style={{ background: "var(--surface-2)", border: "1px solid rgba(255,255,255,0.06)", color: "var(--text-primary)" }} />
          <input value={targetRole} onChange={(event) => setTargetRole(event.target.value)} placeholder="e.g. ML Engineer, Frontend Dev" className="w-full rounded-sm px-3 py-3 bg-transparent font-mono text-sm focus:outline-none" style={{ background: "var(--surface-2)", border: "1px solid rgba(255,255,255,0.06)", color: "var(--text-primary)" }} />
        </div>
        <input value={githubUrl} onChange={(event) => setGithubUrl(event.target.value)} placeholder="https://github.com/username" className="w-full rounded-sm px-3 py-3 bg-transparent font-mono text-sm focus:outline-none" style={{ background: "var(--surface-2)", border: "1px solid rgba(255,255,255,0.06)", color: "var(--text-primary)" }} />

        <PdfResumeUploader value={resumeText} onChange={setResumeText} onTextExtracted={setResumeText} />

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <button onClick={fillExample} className="rounded-sm py-3 font-mono text-[0.62rem] uppercase tracking-[0.18em]" style={{ background: "var(--surface-2)", color: "var(--text-secondary)", border: "1px solid rgba(255,255,255,0.06)" }}>Example Resume</button>
        ...
        <AnimatePresence mode="wait">
          {loading ? <LoadingProgress stepIndex={stepIndex} /> : data ? (
            <motion.div key="results" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="rounded-sm overflow-hidden" style={{ background: "var(--surface-1)", border: "1px solid rgba(255,255,255,0.06)" }}>
              <div className="grid grid-cols-4 gap-1 p-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                {(["score", "gaps", "skills", "profile"] as ResultTab[]).map((tab) => (
                  <button key={tab} onClick={() => setActiveResultTab(tab)} className="rounded-sm py-2 font-mono text-[0.55rem] uppercase tracking-wider" style={{ background: activeResultTab === tab ? "rgba(200,255,0,0.12)" : "transparent", color: activeResultTab === tab ? "#C8FF00" : "var(--text-tertiary)" }}>{tab}</button>
                ))}
              </div>
              <div className="p-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {data.parsed_resume.skills.length ? data.parsed_resume.skills.map((skill) => {
                          ...
                          return <span key={skill} className="rounded-sm px-2.5 py-1.5 font-mono text-[0.55rem] uppercase tracking-wider" style={{ color: inMarket ? "#C8FF00" : "var(--text-tertiary)", background: inMarket ? "rgba(200,255,0,0.07)" : "var(--surface-2)", border: `1px solid ${inMarket ? "rgba(200,255,0,0.18)" : "rgba(255,255,255,0.05)"}` }}>{skill}</span>;
                        }) : <p className="font-mono text-[0.55rem] uppercase tracking-widest" style={{ color: "var(--text-tertiary)" }}>No skills detected. Add a skills section and rerun analysis.</p>}
                      </div>
                    </div>
                    <div>
                        ...
                        {data.matching_skills.length ? data.matching_skills.map((skill: SkillTrend) => (
                          <div key={skill.skill}>
                            <div className="flex justify-between mb-1">
                              ...
                              <span className="font-mono text-[0.55rem]" style={{ color: "var(--text-tertiary)" }}>{Number(skill.demand_score || 0).toFixed(0)}%</span>
                            </div>
                            <DemandBar value={Number(skill.demand_score || 0)} color="#FF6B35" />
                          </div>
                        )) : <p className="font-mono text-[0.55rem] uppercase tracking-widest" style={{ color: "var(--text-tertiary)" }}>No market matches yet. Trigger scrape to populate trends.</p>}
                      </div>
                    </div>
                  </div>
                )}
                {activeResultTab === "profile" && (
                  <div className="space-y-3">
                    <div className="rounded-sm p-4" style={{ background: "var(--surface-2)", border: "1px solid rgba(255,255,255,0.04)" }}>
                      <p className="font-mono text-[0.55rem] uppercase mb-2" style={{ color: "var(--text-tertiary)" }}>Experience Level</p>
                    ...
                    <div className="rounded-sm p-4" style={{ background: "var(--surface-2)", border: "1px solid rgba(255,255,255,0.04)" }}>
                      <p className="font-mono text-[0.55rem] uppercase mb-3" style={{ color: "var(--text-tertiary)" }}>Education</p>
                      <p className="font-body text-sm" style={{ color: "var(--text-primary)" }}>{[education.degree, education.field].filter(Boolean).join(" in ") || "Education not detected"}</p>
                      {education.institution && <p className="font-body text-xs mt-1" style={{ color: "var(--text-secondary)" }}>{education.institution}</p>}
                    </div>
                    <div className="rounded-sm p-4" style={{ background: "var(--surface-2)", border: "1px solid rgba(255,255,255,0.04)" }}>
                      <p className="font-mono text-[0.55rem] uppercase mb-3" style={{ color: "var(--text-tertiary)" }}>Projects</p>
                      <div className="space-y-2">
                        {data.parsed_resume.projects.length ? data.parsed_resume.projects.map((project) => <p key={project} className="font-body text-xs leading-relaxed" style={{ color: "var(--text-secondary)" }}>• {project}</p>) : <p className="font-body text-xs" style={{ color: "var(--text-tertiary)" }}>No projects detected.</p>}
                      </div>
                    </div>
                    <div className="rounded-sm p-4" style={{ background: "var(--surface-2)", border: "1px solid rgba(255,255,255,0.04)" }}>
                      <p className="font-mono text-[0.55rem] uppercase mb-3" style={{ color: "var(--text-tertiary)" }}>Contact</p>
                      <div className="space-y-2">
                        {Object.entries(contact).filter(([, value]) => value).length ? Object.entries(contact).filter(([, value]) => value).map(([key, value]) => {
                          ...
                          return href ? (
                            ...
                            <p key={key} className="font-body text-xs" style={{ color: "var(--text-secondary)" }}>{key}: {value}</p>
                          );
                        }) : <p className="font-body text-xs" style={{ color: "var(--text-tertiary)" }}>No contact fields detected.</p>}
                      </div>
                    </div>
                  </div>
                )}
            ...
            <motion.div key="empty" className="rounded-sm px-6 py-16 flex flex-col items-center justify-center text-center min-h-[520px]" style={{ background: "var(--surface-1)", border: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <p className="font-mono text-[0.6rem] uppercase tracking-[0.25em] mb-2" style={{ color: "var(--text-tertiary)" }}>Results appear here</p>
